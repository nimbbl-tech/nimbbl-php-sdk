...
final class EncryptionTest extends TestCase
{
    ...
    /**
     * Key derivation is deterministic for the same secret — verified behaviorally
     * (no reflection into internals): a ciphertext produced by one instance decrypts
     * on a second instance built from the SAME secret, which is only possible if the
    ...
    public function testSameSecretProducesSameKey(): void
    {
        ...
        $ciphertext = $enc1->encrypt('cross-instance-check');
        $this->assertSame('cross-instance-check', $enc2->decrypt($ciphertext));
    }

    /**
     * Different secrets derive different keys — verified behaviorally: a ciphertext
     * bound to secret A must NOT decrypt under secret B (AES-GCM auth-tag failure),
     * proving the derived keys differ.
     */
    public function testDifferentSecretsProduceDifferentKeys(): void
    {
        $encA = new Encryption('access_secret_key1');
        $encB = new Encryption('access_secret_key2');

        $ciphertext = $encA->encrypt('secret-bound-data');

        $this->expectException(NimbblException::class);
        $encB->decrypt($ciphertext);
    }
