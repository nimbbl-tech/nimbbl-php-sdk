...
final class PayloadHelperUtilsTest extends TestCase
{
    ...
    /**
     * Test globalCloseCheckoutModal (v4 callbackHandler event) is unwrapped like
     * the existing globalHandleCheckoutResponse.
     */
    public function testParsePayloadWithGlobalCloseCheckoutModalUnwrapping(): void
    {
        $actualData = [
        ...
        $payload = json_encode([
            JsonKeys::EVENT_TYPE => JsonKeys::GLOBAL_CLOSE_CHECKOUT_MODAL,
            JsonKeys::PAYLOAD => $actualData,
        ]);

        $result = PayloadHelperUtils::parseResponse($payload, $this->testSecret);

        $this->assertIsArray($result);
        $this->assertEquals('payment_authorized', $result['reason']);
        $this->assertEquals('ord_modal', $result['order_id']);
    }

    /**
     * Test the v4 signed envelope (Base64 payload + signature) is unwrapped to the inner payload.
     */
    public function testParseV4EnvelopeUnwrapsBase64Payload(): void
    {
        $inner = [
            JsonKeys::EVENT_TYPE => 'capture_success',
            JsonKeys::ORDER => [JsonKeys::INVOICE_ID => 'inv_v4'],
            JsonKeys::TRANSACTION => [JsonKeys::TRANSACTION_ID => 't_v4', JsonKeys::TRANSACTION_TYPE => 'capture'],
        ];
        ...
        $envelope = json_encode([
            ...
            JsonKeys::SIGNATURE => hash_hmac('sha256', $innerJson, $this->testSecret),
            JsonKeys::SUB_MERCHANT_ID => 'sm_1',
        ]);

        $result = PayloadHelperUtils::parseResponse($envelope, $this->testSecret);

        $this->assertEquals('capture_success', $result[JsonKeys::EVENT_TYPE]);
        $this->assertEquals('t_v4', $result[JsonKeys::TRANSACTION][JsonKeys::TRANSACTION_ID]);
    }

    /**
     * Test a v4 envelope whose inner payload is itself an encrypted wrapper decrypts correctly.
     */
    public function testParseV4EnvelopeWithEncryptedInner(): void
    {
        $inner = [
            JsonKeys::EVENT_TYPE => 'void_success',
            JsonKeys::ORDER => [JsonKeys::INVOICE_ID => 'inv_enc'],
        ];
        $encrypted = $this->encryption->encrypt($inner);
        $innerWrapperJson = json_encode([JsonKeys::ENCRYPTED_RESPONSE => $encrypted]);

        $envelope = json_encode([
            JsonKeys::VERSION => 'v4',
            JsonKeys::PAYLOAD => base64_encode($innerWrapperJson),
            JsonKeys::NIMBBL_SIGNATURE => 'ignored_here', // parseResponse only unwraps; verification is separate
        ]);

        $result = PayloadHelperUtils::parseResponse($envelope, $this->testSecret);

        $this->assertEquals('void_success', $result[JsonKeys::EVENT_TYPE]);
        $this->assertEquals('inv_enc', $result[JsonKeys::ORDER][JsonKeys::INVOICE_ID]);
    }
}
