...
function loadConfig()
{
    ...
    // Normalize API endpoint from api_host
    if (!empty($config['api_host'])) {
        $config['api_endpoint'] = rtrim($config['api_host'], '/') . '/api/v3';
    } else {
        throw new Exception("Please configure 'api_host' in config.php");
    }
...
function initApi($config)
{
    return new NimbblClient(
        ...
        !empty($config['encrypt_payload']),
        !empty($config['debug_logging']),
        !empty($config['override_log_filename'])
    );
}
